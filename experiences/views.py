...
class Perks(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(PerksSerializer(perk).data)
        else:
...
class PerkDetail(APIView):
    ...
    def put(self, request, pk):
        perk = self.get_object(pk)
        serializer = PerksSerializer(perk, data=request.data, partial=True)
        if serializer.is_valid():
            updated_perk = serializer.save()
            return Response(PerksSerializer(updated_perk).data)
        else:
            return Response(serializer.errors)
